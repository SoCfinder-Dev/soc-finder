...
function ProcessingWidget({ status, setStatus }) {
  return (
    <div
      className={`bg-white w-4xl flex flex-col items-center gap-4\
                       justify-between py-8 rounded-md border-1 \
                       border-dashed border-gray px-44`}
    >
      <div className="flex self-stretch justify-between">
        {status === "processing" && (
          <>
            <p className="text-sm/6 font-bold">
              Chargement de vos FDS
              <span className="after:animate-dots-appear after:white-space-pre"></span>
            </p>
            <div>
              <svg
                aria-hidden="true"
                className="w-4 h-4 text-gray-200 animate-spin dark:text-gray-600 fill-[#2c2421]"
                viewBox="0 0 100 101"
                fill="none"
              >
                <path
        ...
        {status === "processed" && (
          <>
            <p className="text-sm/6 font-bold">Votre analyse est prête</p>
            <button
              className={`flex items-center gap-[0.38rem] px-2 \
          ...
          transition-colors`}
            ...
            >
              <img
                ...
                className="size-[12px]"
              ></img>
              <p className="text-sm text-nowrap">Voir les résultats</p>
            </button>
          </>
        )}
      </div>
      <div
        className={`py-4 px-2.5 bg-[url('/src/assets/textBg.jpg')] \
      bg-cover bg-center bg-blend-lighten bg-[rgba(255,255,255,0.6)] rounded-md`}
      >
        <p className="text-sm text-justify">
          Le règlement REACH (CE n° 1907/2006) dispose que les substances
          préoccupantes (SoC) soient contrôlées et remplacées par des
          alternatives moins dangereuses, lorsque cela est techniquement et
